...
def _safe_save_workbook(wb, filepath, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"Warning: Could not save file after {max_retries} attempts: {e}")
                return False
...
def _read_output(is_chat: bool, output_locator) -> str:
    if is_chat:
        try:
            ...
            if v is not None:
                v = str(v).strip()
                if v and "Transliterated" not in v and "will appear" not in v:
                    return v
        except Exception:
            pass
    try:
        ...
        if v is not None:
            v = str(v).strip()
            # Skip placeholder text and error messages
            if v and "Transliterated" not in v and "will appear" not in v and "Failed to fetch" not in v:
                return v
    except Exception:
        pass
    try:
...
def _check_for_error(page, output_locator) -> bool:
    """Check if there's a 'Failed to fetch' error on the page"""
    try:
        text = output_locator.inner_text()
        if text and "Failed to fetch" in text:
            return True
    except Exception:
        pass
    try:
        # Check for error messages in the page
        error_elements = page.locator("text=/Failed to fetch/i")
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                # Wait for visible content - retry a few times if empty or error
                actual_output = ""
                fetch_error_detected = False
                tries = max(1, int(args.retries))
                for i in range(tries):
                    current = _read_output(is_chat, output_locator)
                    
                    # Check for fetch error and retry
                    if _check_for_error(page, output_locator):
                        fetch_error_detected = True
                        print(f"    Detected 'Failed to fetch' error, retrying... (attempt {i+1}/{tries})")
                ...
                # Handle fetch errors - mark as FAIL or COLLECTED
                if fetch_error_detected and not actual_output:
                    actual_output = "[Fetch Error]"
                ...
                if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                    _safe_save_workbook(wb, args.output)
                ...
                # Mark as FAIL if expected output exists, otherwise COLLECTED
                try:
                    if expected_sinhala:
                        status = "PASS" if actual_output == expected_sinhala else "FAIL"
                    else:
                        status = "COLLECTED" if actual_output else "FAIL"
                    _set_cell_value(ws, row_index, actual_col_idx, actual_output)
                    _set_cell_value(ws, row_index, status_col_idx, status)
